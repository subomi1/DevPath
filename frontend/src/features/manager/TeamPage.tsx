      ...
      <div className="flex-1 flex flex-col p-4 sm:p-6 lg:p-8 max-w-8xl mx-auto w-full min-h-[calc(100vh-4rem)] space-y-6">
        ...
        <div className="bg-surface border border-border rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="bg-canvas border border-border/80 rounded-xl px-4 py-2.5 flex items-center gap-3 flex-1 sm:flex-none">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-ink">{roster.length} Devs</p>
              </div>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="bg-surface border border-border rounded-2xl p-4 shadow-xs space-y-4 animate-pulse">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-canvas rounded-xl shrink-0" />
                    <div className="space-y-2 flex-1">
                      <div className="w-28 h-4 bg-canvas rounded-md" />
                      <div className="w-20 h-3 bg-canvas rounded-md" />
                    </div>
                  </div>
                  <div className="w-full h-2 bg-canvas rounded-full" />
                  <div className="w-24 h-8 bg-canvas rounded-xl ml-auto" />
                </div>
              ))}
            </div>

            {/* Desktop Skeletons */}
            <div className="hidden md:block bg-surface border border-border rounded-2xl p-4 divide-y divide-border/60 shadow-xs">
              {[1, 2, 3, 4].map((i) => (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
              {filtered.map((dev) => (
                <div
                  key={dev.id}
                  className="bg-surface border border-border rounded-2xl p-4 shadow-xs space-y-4 flex flex-col justify-between"
                >
                  <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <span className="inline-flex items-center gap-1.5 bg-canvas border border-border px-2 py-0.5 rounded-lg text-[11px] font-medium text-ink-muted mt-1">
                          <Briefcase size={12} className="text-ink-muted/70" />
                          {dev.job_role}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5 pt-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-semibold text-ink">
                          {dev.overall_progress}% ({dev.overall_progress === 100 ? "Completed" : "In Progress"})
                        </span>
                      </div>
                  ...
                  <div className="pt-2 border-t border-border/60">
                    <Link
                      ...
                      className="w-full inline-flex items-center justify-center gap-1.5 text-xs font-semibold text-primary hover:text-primary/80 bg-primary/10 hover:bg-primary/20 border border-primary/20 px-3 py-2 rounded-xl transition-all shadow-xs"
                    >
                      View Profile
